...
// 필드에 스폰되는 대쉬 해금 픽업. WeaponPickup/MedicalPickup과 동일한 낙하산 연출(공중에서 낙하산을
// 타고 착지 지점으로 떨어진 뒤 스쿼시/파괴 연출)을 그대로 쓰고, 이미지와 픽업 효과만 다르다.
// 체력 회복 대신 GamePlayerController.UnlockDash()를 호출해 대쉬(스프린트)를 영구히 해금한다
// (그 전까지는 Shift가 아무 반응이 없고, 머리 위 게이지도 안 보인다 — GamePlayerController/
// PlayerStaminaBar 참고).
...
public class DashPickup : MonoBehaviour
{
    ...
    [Header("대쉬 해금 이펙트")]
    [Tooltip("대쉬 해금 시 플레이어 위치에서 재생되는 파티클. 비워두면 재생하지 않는다")]
    [SerializeField] private GameObject unlockEffectPrefab;
    [Tooltip("unlockEffectPrefab이 대개 3D 스케일 에셋이라 그대로 쓰면 너무 커서, 여기서 축소 배율을 곱한다")]
    [SerializeField] private float unlockEffectScale = 0.25f;

    [Header("사운드")]
    [Tooltip("대쉬 해금 시 재생되는 사운드. 상자가 획득 직후 곧바로 파괴되므로 자체 AudioSource 대신 PlayClipAtPoint로 재생한다")]
    ...
    // 절차형 낙하산 스프라이트 캐시. WeaponPickup/MedicalPickup과는 별개의 static 필드라 서로 간섭하지 않는다.
    ...
    private void BuildVisualsIfNeeded()
    {
        ...
        // Parachute는 Box의 자식이라 Box의 localScale(boxScale)이 그대로 곱해져 상자를 키우면
        // 낙하산도 같이 커진다. boxScale의 역수를 곱해 상쇄시켜서, 낙하산 크기는 boxScale과
        // 무관하게 항상 원본 크기로 보이게 한다(위치는 상자를 계속 따라간다 — 크기만 분리).
        float parachuteScaleCompensation = boxScale > 0f ? 1f / boxScale : 1f;
        parachuteTransform.localScale = Vector3.one * parachuteScaleCompensation;
    ...
    // 대쉬 해금 시 플레이어 위치에서 재생되는 이펙트.
    // 플레이어를 부모로 붙여서, 파티클이 재생되는 동안 플레이어가 움직여도 같이 따라간다.
    private void SpawnUnlockEffect(Transform player)
    {
        if (unlockEffectPrefab == null) return;

        var effect = Instantiate(unlockEffectPrefab, player.position, Quaternion.identity, player);
        effect.transform.localPosition = Vector3.zero;
        effect.transform.localScale = Vector3.one * unlockEffectScale;
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        ...
        collected = true;
        pc.UnlockDash();
        SpawnUnlockEffect(other.transform);
    ...
    // 대쉬 해금은 이미 끝난 뒤 재생되는 연출이라, 깨진 스프라이트가 없어도 안전하게 즉시 Destroy로 수렴한다.
